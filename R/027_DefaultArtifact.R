...
default_artifact_graph <- function(schemas, defaults, authoring = NULL) {
  if (!identical(defaults[["format_version"]], 2L)) {
    rtemis.core::abort("Unsupported defaults artifact version.", class = "rtemis_schema_error")
  }
    ...
    if (!is.null(publication[["class"]])) {
      ...
      if (identical(publication[["role"]], "family")) {
        for (branch in node[["allOf"]]) {
          ref <- branch[["then"]][["$ref"]]
          if (!is.null(ref)) parents[[ref]] <<- list(identity = identity, selector = branch[["if"]][["properties"]])
        }
      }
    }
    for (i in seq_along(node)) {
      key <- if (is.null(names(node))) as.character(i - 1L) else default_pointer(names(node)[[i]])
      visit(node[[i]], id, paste0(path, "/", key))
    }
  }
  for (id in names(schemas)) visit(schemas[[id]], id)
  decode <- function(value, target) {
    location <- identities[[target]]
    if (is.null(location)) rtemis.core::abort("Unavailable reference target: ", target, ".", class = "rtemis_schema_error")
    schema <- location[["schema"]]
    if (identical(schema[["x-rtemis"]][["publication"]][["role"]], "family")) {
      matches <- Filter(function(branch) {
        selector <- branch[["if"]][["properties"]]
        length(selector) > 0L && all(vapply(names(selector), function(nm) {
          identical(value[[nm]], selector[[nm]][["const"]])
        }, logical(1L)))
      }, schema[["allOf"]])
      if (length(matches) != 1L) rtemis.core::abort("A referenced family requires one explicit discriminator: ", target, ".", class = "rtemis_schema_error")
      target <- locations[[paste0(matches[[1L]][["then"]][["$ref"]], "#")]]
    }
    cls <- class_for(target)
    value[["$schema"]] <- NULL
    unknown <- setdiff(names(value), names(cls@properties))
    if (length(unknown)) rtemis.core::abort("Unknown reference fields: ", paste(unknown, collapse = ", "), class = "rtemis_schema_error")
    document <- attr(cls, "rtemis_artifact_schema")
    args <- lapply(names(value), function(nm) default_from_wire(value[[nm]], document[["properties"]][[nm]], decode))
    names(args) <- names(value)
    do.call(cls, args)
  }
  class_for <- function(identity) {
    if (exists(identity, classes, inherits = FALSE)) return(get(identity, classes, inherits = FALSE))
    if (identity %in% active) rtemis.core::abort("Cyclic default expansion at ", identity, ".", class = "rtemis_schema_error")
    location <- identities[[identity]]
    if (is.null(location)) rtemis.core::abort("Unavailable artifact class: ", identity, ".", class = "rtemis_schema_error")
    ...
    if (!is.null(parent_info)) {
      ...
      schema[["properties"]] <- c(parent_schema[["properties"]], schema[["properties"]])
      declaration <- c(defaults[["declarations"]][[parent_location[["id"]]]], declaration)
      ...
      for (nm in names(parent_info[["selector"]])) {
        ...
        declaration <- c(declaration, default_declarations(get_spec(property), schema[["properties"]][[nm]], paste0("/properties/", default_pointer(nm))))
      }
    }
    # Dispatcher identity has no property annotation and requires supplied
    # input; its concrete branches carry their constant discriminator.
    schema[["properties"]] <- Filter(function(p) !is.null(p[["x-rtemis"]][["type"]]), schema[["properties"]])
    parts <- strsplit(identity, "::", fixed = TRUE)[[1L]]
    policies <- defaults[["resolution"]][[paste0(id, if (nzchar(path)) paste0("#", path) else "")]]
    cls <- JSONSchema_to_S7(schema, name = parts[[2L]], package = parts[[1L]],
      declarations = declaration, authoring = authoring[[id]],
      decode_reference = decode, parent = parent, policies = policies)
    attr(cls, "rtemis_artifact_schema") <- schema
    assign(identity, cls, classes)
    cls
  }
  list(class = function(id) {
    identity <- locations[[paste0(id, if (grepl("#", id, fixed = TRUE)) "" else "#")]]
    if (is.null(identity)) rtemis.core::abort("Unknown artifact schema: ", id, ".", class = "rtemis_schema_error")
    class_for(identity)
  }, decode = decode)
}
...
default_schema_bundle <- function(schemas, root, owner) {
  ...
  localize <- function(node, id) {
    ...
    if (!is.null(ref)) {
      ...
      if (!target %in% names(schemas)) rtemis.core::abort("Unavailable schema reference: ", ref, class = "rtemis_schema_error")
      fragment <- if (grepl("#", ref, fixed = TRUE)) sub("^[^#]*#", "", ref) else ""
