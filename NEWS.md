...
- Breaking change: `SuperConfig` and `SuperConfigLive` no longer have an `algorithm` property, and `setup_SuperConfig()` / `setup_SuperConfigLive()` no longer take an `algorithm` argument. The algorithm is named once, by the `hyperparameters` object, which is the discriminator of the `{algorithm, hyperparameters}` union that the published `supervised` schema references. Write `setup_SuperConfig(hyperparameters = setup_LightRF())`. `train(algorithm = )` is unchanged: it remains a convenience for training with an algorithm's default hyperparameters, and supplying both `algorithm` and `hyperparameters` is still an error only when they name different algorithms.
