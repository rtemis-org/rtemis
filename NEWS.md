...
- **NMF** scores are the non-negative coefficients on the fitted basis, solved with `NMF::fcnnls()`. Values change for every NMF decomposition.
- `setup_NMF(method = )` reaches `NMF::nmf()`; it was declared but never passed, so every fit ran the default.
- `decomp()` with NMF errors when `k` exceeds the rank the fitted basis can reach, rather than returning component scores that are not identified.
...
- The HTML rendering of a `CheckData` object works. It read a property named `class` where the object has `object_class`, so it raised on every call; nothing in the package called it, so nothing surfaced it.
