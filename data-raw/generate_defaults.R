  ...
  inline_declarations <- function(node, pointer = "") {
    ...
    if (!is.null(identity) && identity %in% names(catalog[["inline"]])) {
      ...
      for (nm in names(node[["properties"]])) {
        ...
        if (!is.null(spec)) result <- c(result, default_declarations(spec, node[["properties"]][[nm]],
    ...
    for (i in seq_along(node)) {
      ...
      result <- c(result, inline_declarations(node[[i]], paste0(pointer, "/", key)))
    }
    result
  }
...
schemas <- lapply(file.path(schema_repo, schema_files), jsonlite::fromJSON, simplifyVector = FALSE)
names(schemas) <- vapply(schemas, `[[`, character(1L), "$id")
for (id in names(resolution)) {
  literal <- Filter(function(p) identical(p[["kind"]], "literal"), resolution[[id]])
  if (!length(literal)) next
  cls <- entries[[id]][["cls"]]
  properties <- schemas[[id]][["properties"]]
  base <- family_base(cls)
  if (!is.null(base)) {
    base_id <- schema_reference_urls(catalog, base_url)[[paste0(base@package, "::", base@name)]]
    properties <- c(properties, schemas[[base_id]][["properties"]])
  }
  nms <- intersect(names(literal), names(properties))
  bundle <- default_schema_bundle(schemas, list(type = "object", properties = properties[nms]), id)
  valid <- jsonvalidate::json_validate(
    jsonlite::toJSON(lapply(literal[nms], `[[`, "value"), auto_unbox = TRUE, null = "null", digits = NA),
    jsonlite::toJSON(bundle, auto_unbox = TRUE, null = "null", digits = NA), engine = "ajv", verbose = TRUE)
  if (!isTRUE(valid)) {
    print(attr(valid, "errors"))
    rtemis.core::abort("Invalid resolution literal in ", id, ".", class = "rtemis_schema_error")
  }
}
out <- list(
...
invisible(file.copy("data-raw/defaults-format.schema.json", file.path(out_dir, "format.schema.json"), overwrite = TRUE))
